...
class hr_payslip(models.Model):
    ...
    def _compute_rules_fields(self):
        for line in self.line_ids:
            ...
            if line.code == 'FOODALL':
                self.rule_food_allowance = line.total
    ...
    def _compute_number_of_days(self):
        ...
        if start_work and start_work > __(self.date_to):
            ...
            return
        elif start_work and start_work > __(self.date_from) and __(self.contract_id.first_effective_notice.payslip_date) <= __(self.date_to):
            start_date = __(self.contract_id.first_effective_notice.payslip_date)
        else:
            start_date = __(self.date_from)
        ...
        if self.employee_id and self.date_from and self.date_to:
            ...
            for conflict in conflicts:
                if conflict['start'] > __(self.date_from):
                    ...
                    intersection_start = __(self.dfate_from)
    ...
    def compute_sheet(self):
        ...
        if batch:
            for slip in batch.slip_ids:
                ...
                if old_payslips_ids and not emoployee_old_payslips_ids and slip.employee_id.contract_id and slip.employee_id.contract_id.start_work:
                    start_work = datetime.strptime(slip.employee_id.contract_id.start_work, "%Y-%m-%d")
