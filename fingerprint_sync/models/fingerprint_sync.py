...
class fingerprint_device(models.Model):
    ...
    def get_action_type(self, status):
        ...
        if status_config:
            ...
            raise ValidationError(_("Please configure state for number %s in Fingerprint device %s" % (status, self.name)))
...
class fingerprint_user(models.Model):
    ...
    def create(self, vals):
        ...
        if conn and self._context.get('create', True):
            conn.set_user(uid=int(vals['user_id']), user_id=str(vals['user_id']), name=str(vals['name']), privilege=const.USER_DEFAULT)  # vals['privilege']
