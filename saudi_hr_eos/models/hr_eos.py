...
class employee_eos(models.Model):
    ...
    def action_approve(self):
        ...
        return {
            ...
            'view_id': self.env.ref('hr_eos.pending_transactions_wizard').id,
            'res_id': wizard.id,
            'type': 'ir.actions.act_window',
            'target': 'new',
        }
    ...
    def action_approve_continue(self):
        for rec in self:
            ...
            if not __(rec.actual_last_working_day) or not __(rec.actual_last_working_day_confirmed):
                raise exceptions.ValidationError(
    ...
    def action_final_approve(self):
        for rec in self:
            ...
            return {
                ...
                'view_mode': 'form',
                'res_model': 'pending.transactions.wizard',
                'view_id': self.env.ref('hr_eos.pending_transactions_wizard_final').id,
                'res_id': wizard.id,
                'type': 'ir.actions.act_window',
                'target': 'new',
            }
    ...
    def cancel_pending_transactions(self):
        ...
        if payslips_ids:
            payslips_ids.signal_workflow('cancel_sheet')
        ...
        if hr_leave_ids:
            hr_leave_ids.signal_workflow('refuse')
    ...
    def recalculate_eos(self):
        ...
        self.sudo()._compute_first_hiring_date()
        ...
        if last_allocation and __(last_allocation.allocation_date) and not self.leave_allocation:
            ...
            if diff_months != 0:
                if self.contract_id.annual_leave_policy.lines:
                    start_work = datetime.strptime(self.contract_id.start_work, "%Y-%m-%d")
    ...
    def _compute_last_payslip(self):
        ...
        if __(self.actual_last_working_day):
            if last_payslip and __(self.last_payslip.date_to):
                difference = datetime.strptime(__(self.actual_last_working_day), '%Y-%m-%d') - datetime.strptime(__(self.last_payslip.date_to),
                                                                                                                 '%Y-%m-%d')
            else:
                difference = datetime.strptime(__(self.actual_last_working_day), '%Y-%m-%d') - datetime.strptime(__(self.first_hiring_date),
                                                                                                                 '%Y-%m-%d') + timedelta(1)
